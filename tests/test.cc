...
static uint8_t compute_crc(const std::span<const uint8_t>& span)
{
    uint8_t crc = 0;
...
static void print_frame_bytes(const std::span<const uint8_t>& bytes)
{
    // Trame (LITTLE-ENDIAN) : [0xAA] [0x05] [0x00] [0x80] [0x00 0x00 0x80 0x3f]
    // [CRC]
    //                          start  len_l  len_h  type   payload (float 1.0)
    //                          crc
...
static void test_parser_simple_message()
{
    ...
    const std::vector<uint8_t> trame_output{
        0xAA, // Start
        0x05,
        0x00, // Length = 5 (poids faible d'abord!)
        Frame::MsgType::OUTPUT, // Type
        0x00,
        0x00,
        0x80,
        0x3f, // float 1.0 (little-endian)
    };

    // Calculer le CRC
    const uint8_t crc =
        compute_crc(&trame_output[1], 7); // len_l + len_h + type + payload(4)
    ...
    assert_true(complete.has_value(),
                "Parser reçoit une trame OUTPUT complète");
    assert_true(complete->get_type() == Frame::MsgType::OUTPUT,
                "Type = MSG_OUTPUT");
    assert_true(complete->get_payload_len() == 4, "Payload length = 4");

    const auto payload = complete->get_payload();
    const std::array<std::uint8_t, sizeof(float)> buf{ payload[0], payload[1],
                                                       payload[2], payload[3] };
...
static void test_builder_output()
{
    ...
    const uint8_t crc_calc =
        compute_crc(std::span{ bytes.begin() + 1u, bytes.end() - 1u });
...
static void test_roundtrip()
{
    ...
    std::optional<Frame> complete = std::nullopt;
...
static void test_corrupted_crc()
{
    ...
    auto full_frame = frame.get_full_frame();
    std::vector<uint8_t> bytes{ full_frame.begin(), full_frame.end() };
    ...
    assert_true(!complete.has_value(),
                "Parser ne considère pas la trame comme complète");
}
...
static void test_noise_resilience()
{
    ...
    // Parser reçoit du bruit, puis la trame valide SANS réinitialisation
    // manuelle
    Parser::Parser parser{};
    ...
    assert_true(complete.has_value(),
                "Parser ignore le bruit et valide la vraie trame");
}
